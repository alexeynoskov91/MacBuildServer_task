'''#### <div class="alert alert-info"> Мой вариант работы. Чтобы сделать работу побыстрее не буду тратить время на 
оформление и далее по тексту буду писать в комментариях, к тому же так чуть компактнее </b> </div>'''

# Надеюсь ничего не наврал при копировании, тем более после копирования в .py вносил 
# правки в ноутбук и потом опять сюда в .py
...
# Нелогичностей в виде лидов появившихся до создание рекламы не обнаружено
ads_leads[ads_leads['created_at'] > ads_leads['lead_created_at']]
...
# 1) временная дельта от лида до покупки не отрицательна, т.к. мы вычитали дату лида
# из даты покупки и нам нужно, чтобы дата покупки была больше
...
# Есть дубликаты по паре атрибутов client_id, lead_id
...
# Здесь я делаю left join 
# Либо можно сделать везде inner, но тогда получается в финальную таблицу  
...
sh = gc.open("MacBuildServer")
print(sh.sheet1.get('A01'))


df_fin_gr1 = df_fin_gr.fillna('')

worksheet = sh.sheet1
worksheet.update([df_fin_gr1.columns.values.tolist()] + df_fin_gr1.values.tolist())
